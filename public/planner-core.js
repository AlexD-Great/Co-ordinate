import { buildWeekLabel, runLocalFlowSchedule } from "./flow-forte-local-adapter.js";
...
function buildCoordinationSummary(plans, weeklyLoad, settings, weekBase) {
  const weeklyView = Array.from({ length: 12 }, (_, index) => {
    ...
    return {
      label: buildWeekLabel(weekBase, index),
      totalHours,
...
export function rebalanceState(inputState) {
  ...
  const { scheduledPlans, weeklyLoad, weekBase } = runLocalFlowSchedule(orderedPlans, settings);
